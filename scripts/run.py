...
def make_image_list(data_path, factor):
    ...
    if 0.999 < factor < 1.001:
        for suf in suffix:
            image_list += glob(os.path.join(data_path, "images", suf)) + glob(
                os.path.join(data_path, "images_1", suf)
            )
    else:
...
def make_depth_list(data_path):
    depth = read_depth_from_bin(data_path)
    depth_to_file(depth, data_path)
...
def main(conf: DictConfig) -> None:
    ...
    if conf["dataset"]["with_depth"]:
        make_depth_list(data_path)
